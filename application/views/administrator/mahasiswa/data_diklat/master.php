        ...
        <div class="portlet light" style="position:relative">
            <div class="portlet-title">
                ...
                <div class="actions" id="tombol-action" style="display:block; margin-left:5px;">
                    <a class="btn blue btn-sm" href="<?=site_url('administrator/mhs_dir/data_diklat/print_excel/'.$mahasiswa->id_sdm)?>">
                ...
                <div class="table-responsive">
                    <table id="<?=$controller?>_table" class="table table-bordered table-striped table-hover" style="width:100%">
                        ...
                        <?php 
                          ...
                          foreach ($this->Content_m->select_db('sispeg_tr_pegawai_diklat')->result() as $key ) { ?>
                            <tr>
                              <td><?=$key->jenis_diklat?></td>
                              <td><?=$key->tmpt_diklat?></td>
                              <td><?=$key->tgl_mulai?></td>
                              <td><?=$key->tgl_selesai?></td>                             
                              <td>
                                <a href="<?=site_url()?>/administrator/mhs_dir/data_diklat/edit_data/<?=$mahasiswa->id_sdm?>/<?=$key->id_sdm_diklat?>">
                                  <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
                                </a>
                                <a data-toggle="tooltip" data-placement="top" title="Hapus" href="<?=site_url()?>/administrator/mhs_dir/data_diklat/del_data/<?=$key->id_sdm_diklat?>" onclick="return confirm('Are you sure?')" style="cursor:pointer">
